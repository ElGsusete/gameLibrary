...
export function GamesPage() {
  ...
  useEffect(() => {
    ...
    const initialPlatform =
      platformsParam.trim().length > 0
        ...
        : [];
    const initialSort =
      (searchParams.get("sort") as "date" | "score" | "title" | null) ?? "date";
  ...
  const updateSearchParams = (
    ...
    nextSort: "date" | "score" | "title",
  ) => {
    setSearchParams(
      (prev) => {
        const next = new URLSearchParams(prev);
        if (nextPlatforms.length > 0)
          next.set("platforms", nextPlatforms.join(","));
        else {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-8">
      <h1 className="mb-6 text-center text-3xl font-bold text-cp-light">
        Todos los juegos
      </h1>
      ...
      <div className="mb-4 flex flex-col gap-3 md:hidden">
        <div>
          <span className="mb-1 block text-xs font-medium uppercase tracking-wide text-cp-muted">
            Plataforma
          </span>
            ...
            <button
              ...
              className={`shrink-0 rounded-full border px-3 py-1 text-xs ${
                platformFilter.length === 0
                  ...
                  : "border-cp-surface bg-cp-dark text-cp-light"
              }`}
            >
              Todas
            </button>
            {platforms.map((p) => {
              ...
              return (
                <button
                  ...
                  className={`shrink-0 rounded-full border px-3 py-1 text-xs flex items-center gap-1.5 ${
                    active
                      ? "border-cp-neon bg-cp-neon text-cp-black"
                      : "border-cp-surface bg-cp-dark text-cp-light"
                  }`}
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 ${
                    active ? "bg-cp-neon text-cp-black" : "text-cp-light"
                  }`}
                  aria-pressed={active}
                >
                  {opt.label}
      ...
      {/* Filtros escritorio: controles avanzados */}
