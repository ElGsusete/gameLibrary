...
export function Header() {
  ...
  const searchDropdown = showSearchDropdown && searchResults.length > 0 && (
    <ul className="absolute left-0 right-0 top-full z-50 mt-1 max-h-72 overflow-y-auto rounded-lg border border-cp-surface bg-cp-dark py-1 shadow-xl">
      {searchResults.map((g) => (
        <li key={g.id}>
          <button
            type="button"
            ...
            className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-cp-light hover:bg-cp-surface"
          >
            {g.coverImage ? (
              <img
                src={g.coverImage}
              ...
              <div className="h-8 w-12 shrink-0 rounded bg-cp-surface" />
            )}
    ...
    <header className="sticky top-0 z-50 border-b border-cp-surface bg-cp-dark/98 backdrop-blur" ref={mobileMenuRef}>
      <div className="relative flex h-14 w-full items-center gap-2 px-4">
        ...
        >
          ...
          <div className="relative flex w-full min-w-0 items-center rounded-lg border border-cp-surface bg-cp-dark md:hidden">
            <div className="relative flex min-w-0 flex-1">
              ...
              <input
                ...
                className="w-full rounded-lg border-0 bg-transparent py-1.5 pl-8 pr-2 text-sm text-cp-light placeholder-cp-muted focus:outline-none focus:ring-0"
              />
            </div>
            <div className="h-6 w-px shrink-0 bg-cp-surface" aria-hidden />
            <button
              type="button"
              onClick={() => setMobileMenuOpen((v) => !v)}
              className="flex shrink-0 items-center justify-center px-3 py-2 text-cp-muted hover:text-cp-neon transition-colors"
              ...
              >
                <li>
                  <Link
                    ...
                    className="flex items-center gap-2 px-3 py-2 text-sm text-cp-light hover:bg-cp-surface hover:text-cp-neon transition-colors"
                  >
      ...
      >
        <nav className="border-t border-cp-surface bg-cp-dark/95 px-4 py-3 backdrop-blur">
          <ul className="flex flex-col gap-0.5">
            <li>
              <Link
                to="/"
                onClick={closeMobileMenu}
                className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-cp-light hover:bg-cp-surface hover:text-cp-neon transition-colors"
              >
                <Home className="h-5 w-5 shrink-0" />
                Inicio
              </Link>
            </li>
            <li className="border-t border-cp-surface pt-1 mt-1">
              <span className="px-3 py-1.5 text-xs font-medium uppercase tracking-wider text-cp-muted">
                Juegos
              </span>
            </li>
            <li>
            ...
            <li className="border-t border-cp-surface pt-1 mt-1">
              <Link
                to="/add-game"
                onClick={closeMobileMenu}
                className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-cp-neon hover:bg-cp-surface transition-colors"
              >
            ...
            <li className="border-t border-cp-surface pt-1 mt-1">
              {isLoggedIn ? (
                <button
                  ...
                  className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-cp-light hover:bg-cp-surface hover:text-cp-neon transition-colors"
                >
